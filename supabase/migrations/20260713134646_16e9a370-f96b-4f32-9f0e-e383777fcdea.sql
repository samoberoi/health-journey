...
BEGIN
  ...
  -- Allowed diet_type values per preference
  IF _is_vegan THEN
  ...
  FOR _d IN 0..29 LOOP
    -- Mixed alternates veg/nonveg proteins; pure non-veg uses nonveg on most days
    _use_nonveg := CASE
    ...
    FOREACH _slot IN ARRAY _slots LOOP
      ...
      IF _slot = 'breakfast' THEN
        -- protein + grain alt + healthy fat
        _items := _items || COALESCE((
          ...
          ORDER BY md5(id::text || _d::text || 'bp') LIMIT 1
        ), 'Eggs');
        _items := _items || COALESCE((
          SELECT name FROM public.food_items
          WHERE is_active AND recommendation IN ('encourage','moderate')
          ...
          ORDER BY md5(id::text || _d::text || 'bg') LIMIT 1
        ), 'Oats');
        _items := _items || COALESCE((
          SELECT name FROM public.food_items
          WHERE is_active AND recommendation IN ('encourage','moderate')
          ...
          ORDER BY md5(id::text || _d::text || 'bf') LIMIT 1
        ), 'Olive Oil');

      ELSIF _slot = 'lunch' THEN
        _items := _items || COALESCE((
          SELECT name FROM public.food_items
          WHERE is_active AND recommendation IN ('encourage','moderate')
            AND diet_type::text = ANY(_diet_filter)
        ...
        _items := _items || COALESCE((
          ...
          WHERE is_active AND recommendation IN ('encourage','moderate')
            AND diet_type::text = ANY(_diet_filter)
            AND filter_id = _f_alt_grain
          ORDER BY md5(id::text || _d::text || 'lg') LIMIT 1
        ), 'Cauliflower Rice');
        _items := _items || COALESCE((
          SELECT name FROM public.food_items
          WHERE is_active AND recommendation IN ('encourage','moderate')
      ...
      ELSIF _slot = 'snack' THEN
        _items := _items || COALESCE((
          SELECT name FROM public.food_items
          WHERE is_active AND recommendation IN ('encourage','moderate')
            AND diet_type::text = ANY(_diet_filter)
            AND filter_id = _f_nuts
          ORDER BY md5(id::text || _d::text || 'sn') LIMIT 1
        ), 'Almonds');
        _items := _items || COALESCE((
          SELECT name FROM public.food_items
          WHERE is_active AND recommendation IN ('encourage','moderate')
          ...
          ORDER BY md5(id::text || _d::text || 'sd') LIMIT 1
        ), 'Greek Yogurt');
      ...
      ELSE -- dinner
        _items := _items || COALESCE((
          ...
          WHERE is_active AND recommendation IN ('encourage','moderate')
            AND diet_type::text = ANY(_diet_filter)
            AND filter_id = CASE WHEN _use_nonveg AND NOT _is_vegan THEN _f_protein_nv ELSE _f_protein_veg END
          ORDER BY md5(id::text || _d::text || 'dp') LIMIT 1
        ), 'Tofu');
        _items := _items || COALESCE((
          SELECT name FROM public.food_items
          WHERE is_active AND recommendation IN ('encourage','moderate')
            AND diet_type::text = ANY(_diet_filter)
            AND filter_id = _f_veg
          ORDER BY md5(id::text || _d::text || 'dv') LIMIT 1
        ), 'Broccoli');
        _items := _items || COALESCE((
          SELECT name FROM public.food_items
          WHERE is_active AND recommendation IN ('encourage','moderate')
            AND diet_type::text = ANY(_diet_filter)
            AND filter_id = _f_fats
          ORDER BY md5(id::text || _d::text || 'df') LIMIT 1
        ), 'Avocado');
      END IF;
