...
-- 1. Reset pillar config to match the frontend "Today's Habits" list exactly.
UPDATE public.global_streak_config
SET pillars = '[
  ...
  {"key":"yoga","label":"Yoga & Stress","required":true},
  {"key":"water","label":"Water","required":true,"threshold":8},
  {"key":"diabetes","label":"Blood Sugar Log","required":true}
]'::jsonb,
    updated_at = now();

-- 2. Rewrite the streak compute function. Pillars that don't apply to the user
--    (no fasting plan, no supplement plan, no yoga bookings, no diabetes flag)
--    are silently skipped so `all_complete` reflects reality for THIS user.
...
DECLARE
  ...
  -- App settings (fallbacks match production defaults)
  _yoga_goal int := 20;
  ...
  -- Movement target from user's current level (fallback 5000)
...
BEGIN
  ...
  -- Load app settings
  SELECT COALESCE(NULLIF(value,'')::int, 20) INTO _yoga_goal
    FROM public.app_settings WHERE key = 'daily_yoga_minutes';
  SELECT COALESCE(NULLIF(value,'')::int, 5) INTO _exercise_goal
  ...
  FOR _p IN SELECT * FROM jsonb_array_elements(_pillars) LOOP
    ...
    ELSIF _key = 'supplements' THEN
      ...
      IF _applicable THEN
        -- All planned items for today marked taken
        SELECT (
    ...
    ELSIF _key = 'yoga' THEN
      ...
      IF _applicable THEN
        ...
        -- OR video-progress-based hit against the daily minute goal
        IF NOT _hit THEN
  ...
  -- Latest weight/glucose for snapshot
